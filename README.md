...
- **Operator console** (`/app/operator`) — a third **Operator** role (toggle in the app header) for
  ...
  The console is **passcode-gated** (demo passcode `csd-operator`) so it isn't reachable just by
  flipping the role toggle. Operator actions are **consequential on the public side**: suspending a
  provider removes it from Discover and the match flow (and shows a notice on its profile), and
  removing a flagged review drops it from the listing page **and the profile's headline rating** —
  the trust-&-safety loop closes live.
